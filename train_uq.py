...
class Trainer:
    ...
    # def get_mask(self, coords: torch.Tensor, base_jump: int = 0):
    ...
    def train_one_epoch(self, epoch: int):
        ...
        while batch is not None:
            ...
            # if self.ld is not None:
            #     if 'fourier' in self.configs.ed_name or 'inr' in self.configs.ed_name:
        ...
        for epoch in range(start_epoch, nepochs + 1):
            ...
            if self.rank == 0:
                ...
                if (epoch + 0) % 10 == 0:
                    ...
                    self.logger.info(f'Epoch {epoch}, train_loss={loss_uq.cpu()}')
                    if loss_uq < eval_best:
                        eval_best = loss_uq + 0.
                        self._save_ckpt(configs.ckpt_path)
                        self.logger.info(f'Epoch {epoch}, train_best={eval_best.cpu()}, saved to ckpt.')

    def test(self, configs: LAConfigs, evaluator: Evaluator):

        nepochs = configs.train['nepochs']
        try:
            self.load_ckpt(configs.ckpt_path)
            self.logger.info(f'Checkpoint loaded from {configs.ckpt_path} successfully.')
        except:
            self.logger.warning(f'No checkpoint found at {configs.ckpt_path}, training from scratch.')
    ...
    def estimate_embedding_err(self) -> None:

        self.load_ckpt()

        self.ed.eval()
        self.encoder_cache.eval()
        self.ed.requires_grad_(False)
        self.encoder_cache.requires_grad_(False)
        if self.ld is not None:
            self.ld.eval()
        ...
        for k, batch in enumerate(self.dataloader):
            ...
            # Forward pass through the models
            # Use the latent_states tensor as an initial state
            zz = self.encoder_cache(seq_idxs)

            # def decode4jac(z: torch.Tensor):
            #     z_unsqueeze = z[None, None, ...]
            #     x_unsqueeze = self.ed(
            #         z_unsqueeze, operation='decode', coords=coords[:, None],
            #         coords_ang=coords_ang[:, None])
            #     x = x_unsqueeze[0, 0, ...]
            #     return x

            # for z, x in zip(zz.view(-1, zz.shape[-1]), field_true.view(-1, field_true.shape[-1])):

            #     jac = torch.autograd.functional.jacobian(decode4jac, z)
            #     print(denominator)
            #     x_rec = decode4jac(z)
            #     err_x = x_rec - x
            #     err_z = torch.linalg.lstsq(jac, err_x).solution
            #     err_cov = torch.outer(err_z, err_z)
            #     err_cov_acc = err_cov_acc + err_cov

            #     denominator += 1.

            zz_pert = zz + 1e-6 * torch.randn_like(zz)

            xx_rec = self.ed(zz, operation='decode', coords=coords[:, None],
                             coords_ang=coords_ang[:, None])
            xx_pert_rec = self.ed(zz_pert, operation='decode', coords=coords[:, None],
                                  coords_ang=coords_ang[:, None])

            xx_pert = xx_pert_rec - xx_rec
            xx_pert = xx_pert.reshape(-1, xx_pert.shape[-1])
            xx_pert_std = torch.std(xx_pert, dim=0) / 1e-6
            print(xx_pert_std.shape)
            print(xx_pert)
            print(xx_pert_std.mean())

        # err_cov_acc = err_cov_acc / denominator
        # print(err_cov_acc)
        # print(err_cov_acc.shape)
        # print(torch.diagonal(err_cov_acc))
