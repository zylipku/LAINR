...
from dataset import DINoShallowWater3, ERA5v2
...
def main(args):
    ...
    uncertainty = get_uncertainty(logger, name=uq_name, **configs.uq["params"])
    encoder_decoder = encoder_decoder.to(device)
    latent_dynamics = latent_dynamics.to(device)
    uncertainty = uncertainty.to(device).eval()
    ...
    uncertainty.load_state_dict(transform_state_dict(ckpt['uq']))
    ...
    def decoder4jac(z: torch.Tensor):
        return encoder_decoder.decode(z, coords=coords[0], coords_ang=coords_ang[0])
    ...
    with torch.no_grad():
        ...
        prefix = f'{method_name}_{sigma_x_b=}_{sigma_z_b=}_mod_sigma=diagonal_{n_obs=}_'
        ...
        results = xps.evaluate(xx_t[obs_t_idxs],
                               ...
                               device=device)
        logger.info(f'RESULTS: {sigma_x_b=}, {sigma_z_b=}, mod_sigma=diagonal, {n_obs=} | {results}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--uq', type=str, default=uq, help='uncertainty name.')
    parser.add_argument('--sigma-x-b', type=float, default=1e-1, help='sigma_x_b')
    parser.add_argument('--sigma-xz-b-ratio', type=float, default=1e-1, help='sigma_xz_b_ratio')
