...
class ShallowWaterNormalized(LADataset):
    ...
    def __init__(self,
        ...
        if group == 'train' or group == 'train_eval':
            self.traj_ids = [str(umax) for umax in self.train_umax]
            self.valid_timesteps = self.valid_train_timesteps
            self.cached_data_path = os.path.join(self.ROOT_PATH, 'cached_normalized_train_data.pt')
        elif group == 'test':
            self.traj_ids = [str(umax) for umax in self.test_umax]
            self.valid_timesteps = self.valid_test_timesteps
            self.cached_data_path = os.path.join(self.ROOT_PATH, 'cached_normalized_test_data.pt')
        else:
            raise ValueError(f'group must be "train" or "test", but got {group}')
...
class ShallowWaterPtNormalized(LADataset):
    ...
    def __init__(self,
        ...
        self.height_mean = torch.load(os.path.join(self.ROOT_PATH, self.height_mean_file_name))[None, ::2, ::2]
        self.height_std = torch.load(os.path.join(self.ROOT_PATH, self.height_std_file_name))[None, ::2, ::2]
        self.vorticity_mean = torch.load(os.path.join(self.ROOT_PATH, self.vorticity_mean_file_name))[None, ::2, ::2]
        self.vorticity_std = torch.load(os.path.join(self.ROOT_PATH, self.vorticity_std_file_name))[None, ::2, ::2]
