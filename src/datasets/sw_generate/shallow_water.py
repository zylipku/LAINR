...
u_max = np.random.uniform(60, 80)  # [60, 80] for u_max
alpha = 1 / 3.
beta = 1 / 15.
# alpha = 1 / np.random.uniform(2, 4) # [1/4, 1/2] for alpha
# beta = 1 / np.random.uniform(10, 20) # [1/20, 1/10] for beta
...
def zcross(A): return d3.MulCosine(d3.skew(A))


# Initial conditions: zonal jet
phi, theta = dist.local_grids(basis)
lat = np.pi / 2 - theta + 0 * phi
umax = u_max * meter / second
lat0 = np.pi / 7
lat1 = np.pi / 2 - lat0
...
# Analysis
snapshots = solver.evaluator.add_file_handler(f'seedno={seedno}', iter=6, max_writes=1010)
