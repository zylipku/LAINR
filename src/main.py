...
def main():
    ...
    assert len(sys.argv) >= 2, \
        "Please specify the phase. Usage: python main.py --phase=[pretrain|finetune] <optional args>"

    assert sys.argv[1] in ['--phase=pretrain', '--phase=finetune', '--phase=postproc', '--phase=assimilate'], \
        "Please specify the phase. Usage: python main.py --phase=[pretrain|finetune] <optional args>"
    phase = sys.argv[1]
    ...
    if phase == '--phase=postproc':
        # print in red text
        print("\033[93m" + "-" * 30 + "\033[0m")
        print("\033[94mPost-processing\033[0m with the following arguments:")
        if len(sys.argv) == 2:
            print("<None>")
        else:
            print(', '.join(sys.argv[2:]))
        print("\033[93m" + "-" * 30 + "\033[0m")
        subprocess.run([sys.executable, 'postproc_main.py'] + sys.argv[2:])
