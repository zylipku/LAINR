...
class ShallowWaterOffGrid(DatasetPacker):
    ...
    def packing_from_raw(self, group: str) -> MetaData:
        ...
        for idx, traj_id in enumerate(traj_ids):
            ...
            height_np = traj_file['height'][:][self.trun_timeslice, 1:: 2, 1:: 2]  # (240, 128, 64)
            vorticity_np = traj_file['vorticity'][:][self.trun_timeslice, 1:: 2, 1:: 2]  # (240, 128, 64)
            ...
            if idx == 0:
                phi_np = traj_file['phi'][:][1:: 2]  # (256,) -> (128,)
                theta_np = traj_file['theta'][:][1:: 2]  # (128,) -> (64,)
