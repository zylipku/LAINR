...
class XPS:
    ...
    def run(self,
        ...
        for method in self.methods:
            ...
            # def is_h5py_object(obj):
            #     return isinstance(obj, h5py.File) or isinstance(obj, h5py.Group) or isinstance(obj, h5py.Dataset)

            # def find_h5py_objects(obj, obj_name=''):
            #     h5py_objects = []
            #     if is_h5py_object(obj):
            #         h5py_objects.append((obj_name, obj))
            #     elif isinstance(obj, dict):
            #         for key in obj:
            #             sub_obj_name = f"{obj_name}.{key}" if obj_name else key
            #             h5py_objects.extend(find_h5py_objects(obj[key], sub_obj_name))
            #     elif isinstance(obj, list):
            #         for i, item in enumerate(obj):
            #             sub_obj_name = f"{obj_name}[{i}]"
            #             h5py_objects.extend(find_h5py_objects(item, sub_obj_name))
            #     elif hasattr(obj, '__dict__'):
            #         for attr_name in obj.__dict__:
            #             sub_obj_name = f"{obj_name}.{attr_name}" if obj_name else attr_name
            #             h5py_objects.extend(find_h5py_objects(obj.__dict__[attr_name], sub_obj_name))
            #     return h5py_objects

            # h5py_objects = find_h5py_objects(results)
            # if h5py_objects:
            #     for var_name, var_obj in h5py_objects:
            #         print(f"{var_name} is an h5py object")
