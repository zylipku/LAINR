...
class DA(nn.Module):
    ...
    def __init__(self,
                 ...
                 default_covM: torch.Tensor | Callable[[torch.Tensor], torch.Tensor] = None,
        ...
        if isinstance(default_covM, torch.Tensor):
            self.covM = default_covM
            self.covM_chol = torch.linalg.cholesky(self.covM)
            self.uq = None
        else:  # or uq
            self.covM = None
            self.covM_chol = None
            self.uq = default_covM
    ...
    def _get_covM_chol(self, x_a_prev_info: Any) -> torch.Tensor:
        '''
        ...
        Returns:
            torch.Tensor: covM
        '''
        x_a_prev = self._x_info_to_a(x_a_prev_info)
        if self.covM_chol is None:
            with torch.no_grad():
                covM_chol = self.uq(x_a_prev, output='cov_chol_mat')
        else:
            covM_chol = self.covM_chol.to(x_a_prev)
    ...
    def assimilate(self,
                   ...
                   covM: torch.Tensor = None, covH: torch.Tensor = None,
        ...
        for assim_step in tqdm(range(assim_length + 1), desc=self.tqdm_title):  # range(assim_length + 1):
            ...
            covM_chol = self._get_covM_chol(x_a_prev_info=x_info)
            # self.logger.info(torch.diagonal(covM_chol))
