...
class ETKF_Q_L(EnMethod):
    '''
    Ensemble Transformed Kalman Filter - Q on Latent Space
    ...
    def __init__(self, logger: logging.Logger,
                 mod_dim: int, obs_dim: int, lat_dim: int, ens_dim: int,
                 latent_dynamics: nn.Module,
                 decoder: nn.Module,
                 opH: Operator = None,
                 artificial_dyn_cov: torch.Tensor = None,
                 artificial_obs_cov: torch.Tensor = None,
                 infl: float = 1,
                 dtype=None, device=None,
                 **kwargs) -> None:

        super().__init__(logger, mod_dim, obs_dim, ens_dim,
                         default_opM=Operator.compose(latent_dynamics),
                         default_opH=Operator.compose(decoder, opH),
                         default_covM=artificial_dyn_cov,
                         default_covH=artificial_obs_cov,
                         infl=infl, dtype=dtype, device=device)
    ...
    def _forecast(self,
        ...
        if additional_n_zeros > 0:
            x_b_dev = torch.cat([x_b_dev, torch.zeros(*x_b_dev.shape[:-1], additional_n_zeros)], dim=-1)
