    ...
    def load_traj_data(self):
        ...
        try:
            ...
            for idx, traj_id in enumerate(self.traj_ids):
                ...
                if self.offgrid:
                    height = np.load(os.path.join(self.ROOT_PATH, f'traj_{traj_id}_height_offgrid.npy'))
                    vorticity = np.load(os.path.join(self.ROOT_PATH, f'traj_{traj_id}_vorticity_offgrid.npy'))
                else:
                ...
                if idx == 0:
                    if self.offgrid:
                        phi = np.load(os.path.join(self.ROOT_PATH, f'traj_{traj_id}_phi_offgrid.npy'))
                        theta = np.load(os.path.join(self.ROOT_PATH, f'traj_{traj_id}_theta_offgrid.npy'))
                    else:
