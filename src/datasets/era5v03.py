...
class ERA5v03(DatasetPacker):

    '''

    ERA5 1h interval, 1981-2015 (35 years), first 1440 snapshots (60 days)
    ...
    timestep_slice_tr = slice(0, 24 * 60)
    timestep_slice_va = slice(24 * 25, 24 * 35)
    timestep_slice_ts = slice(24 * 25, 24 * 35)
    ...
    def packing_from_raw(self, group: str) -> MetaData:
        ...
        timestep_slice = getattr(self, f'timestep_slice_{group}')
        ...
        for year_id in year_ids:
            ...
            z500 = torch.from_numpy(z500_np[timestep_slice]).transpose(-2, -1).to(self.dtype)
            t850 = torch.from_numpy(t850_np[timestep_slice]).transpose(-2, -1).to(self.dtype)
