...
from trainer.postproc import PostProcer


def conf_prepare(cfg: PostProcConfig):
    ...
    cfg.uncertainty_est.ndim = cfg.encoder_decoder.latent_dim


def main_worker(rank, num_gpus: int, cfg: PostProcConfig):
    ...
    # mlflow --------------------------------------------------------
    if rank == 0:
        mlflow.set_experiment('postproc_' + cfg.dataset.name + '_' + cfg.encoder_decoder.model_name)
    ...
    dataloader_tr = DataLoader(dataset_tr, batch_size=cfg.bs, sampler=sampler_tr, num_workers=4)
    dataloader_va = DataLoader(dataset_va, batch_size=cfg.bs, sampler=sampler_va, num_workers=4)
    dataloader_ts = DataLoader(dataset_ts, batch_size=cfg.bs, sampler=sampler_ts, num_workers=4)
    ...
    uncertainty_est = DDP(uncertainty_est, device_ids=[rank])
    ...
    trainer = PostProcer(logger=logger,
...
cs.store(name="postproc_schema", node=PostProcConfig)
...
cs.store(name="uncertainty_est_schema", group='uncertainty_est', node=UEConfig)
cs.store(name="dataset_schema", group='dataset', node=DatasetConfig)


@hydra.main(config_path="configs/postproc", config_name="config", version_base='1.2')
def main_pretrain(cfg: PostProcConfig):
